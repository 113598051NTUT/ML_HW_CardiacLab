...
class SegmentationCardiac(TaskConfig):
    ...
    def init(self, name: str, model_dir: str, conf: Dict[str, str], planner: Any, **kwargs):
        ...
        # Network name
        self.network_name = self.conf.get("network", "unetcnx_x0, unetcnx_a1, testnet")

        # Model Files
        self.path = [
            os.path.join(self.model_dir, self.network_name, f"best_model_whmyo.pth"),  # pretrained
            os.path.join(self.model_dir, self.network_name, f"out_model_whmyo.pth"),  # published
        ]
